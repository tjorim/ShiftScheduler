...
### Widget Code Pattern
```typescript
// ScheduleGrid receives typed ActionValues (auto-generated by Mendix)
...
// Direct action execution with raw values (Mendix handles Option wrapping internally)
if (onEditEvent?.canExecute && !onEditEvent.isExecuting) {
    onEditEvent.execute({ eventId: event.id }); // pass raw string, not Option
}

if (onCreateEvent?.canExecute && !onCreateEvent.isExecuting) {
    onCreateEvent.execute({ personId: person.id, date: dateString }); // pass raw strings
}
```
