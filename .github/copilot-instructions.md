...
- **Import for testing**: Import the generated `dist/1.14.1/asml.ShiftScheduler.mpk` file into Mendix Studio Pro for testing.
- **Validate build output**: Check that `dist/1.14.1/asml.ShiftScheduler.mpk` exists and is ~400KB after build.
...
- **ALWAYS** run the complete build sequence after making changes:
  ...
  3. Check `ls -lh dist/1.14.1/asml.ShiftScheduler.mpk` shows ~400KB file
...
# Verify build output (~403KB)
ls -lh dist/1.14.1/asml.ShiftScheduler.mpk
